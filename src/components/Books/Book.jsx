...
const Book = ({ book }) => {
  ...
  return (
    <Link to={`/bookdetails/${bookId}`}>
      <div className="card bg-base-100 shadow-xl border-2 hover:cursor-pointer hover:shadow-yellow-100">
        ...
        <div className="card-body">
          ...
          <div className="flex justify-between font-medium work-sans">
            <p>{category}</p>
            <p className=" text-end flex justify-end gap-2 items-center">
              {rating} <TiStarOutline className="text-2xl" />
            </p>
          </div>
        </div>
      </div>
